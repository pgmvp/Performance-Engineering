...
split_into_blocks(const std::vector<T>& channel, int d1, int d2)
{
    ...
    int additional_size = d1 % blocks_num;
...
int main()
{
    ...
    unsigned char** image = LoadBitmapFile3C("C:\\Users\\Max\\Documents\\QT\\Homework3\\lenna.bmp",
    bitmapInfoHeader);
...
//    std::vector<long> vec(channel, channel + image_size / 3);
    ...
    //***************INTEGRAL IMAGE***************
    auto res = integral_image(vec, image_width, image_height);
